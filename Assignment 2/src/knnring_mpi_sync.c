...
knnresult distrAllkNN(double * X, int n, int d, int k) {
	...
	int nproc, pid;
	MPI_Status status, sendstat;
	MPI_Request sendreq;
	int tag = 50;

	//Timer for communications
	double p_time = 0;
	...
	for(int p=0; p<nproc; p++) {
		...
		//!--- And then start the procedure of sending and receiving data points in a ring fashion---! 
		...
		//Send your block of points Z to the next process
		MPI_Isend(Z, buffLen, MPI_DOUBLE, (pid+1)%nproc, tag, MPI_COMM_WORLD, &sendreq);

		//Receive the new block of points Z from previous process
		MPI_Recv(tempZ, cap*d, MPI_DOUBLE, (nproc+pid-1)%nproc, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
		MPI_Get_count(&status, MPI_DOUBLE, &buffLen);
